...
export function LeagueMarquee() {
    ...
    return (
        <div className="w-full bg-black/50 py-8 md:py-12 border-y border-white/5 relative overflow-hidden isolate">
            <div className="absolute inset-0 bg-gradient-to-r from-black via-transparent to-black z-10 pointer-events-none" />

            <div className="container px-4 mx-auto mb-6 md:mb-8 relative z-20">
            ...
            <div className="flex overflow-hidden">
                <motion.div
                    ...
                    className="flex gap-10 md:gap-24 items-center whitespace-nowrap px-6 md:px-12"
                >
                    {displayLeagues.map((league, i) => {
                        ...
                        return (
                            <div key={i} className="flex items-center gap-4 group grayscale hover:grayscale-0 transition-all duration-300 opacity-40 hover:opacity-100">
                                <div className={`relative w-8 h-8 md:w-12 md:h-12 ${isSoccerLeague ? 'brightness-[3] contrast-[1.2] scale-110' : ''}`}>
                                    <Image
                                        src={league.logo}
                                        alt={league.name}
                                        fill
                                ...
                                <span className="text-zinc-400 font-black text-base md:text-2xl tracking-tighter group-hover:text-white transition-colors">
                                    {league.name}
                                </span>
                            </div>
                        );
